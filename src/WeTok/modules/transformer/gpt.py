...
class GPT(nn.Module):
    def __init__(self, vocab_size, block_size, spatial_n_layer=12, n_head=8, dim=256, cond_dim=256, factorized_n_layer=2,
        ...
        super().__init__()
        assert factorized_k == len(factorized_bits), "factorized_k should be same as the length of factorized_bits"
        ...
        if token_factorization:
            if factorized_k == 2:
                ...
                for i, factorized_bit in enumerate(factorized_bits):
                    emb = nn.Embedding(2**factorized_bit, self.config.dim)
    ...
    def forward(
        ...
        if self.token_factorization:
            if self.config.factorized_k == 2:
                ...
                cls_token_embeddings = self.class_emb(idx[-1], train=self.training)[:, :self.config.cls_token_num]
                token_embeddings = torch.concat([cls_token_embeddings, token_embeddings[:, :-1, :]], dim=1)
                h = self.token_drop(token_embeddings)
        else:
        ...
        if self.config.factorized_k == 2:
            ...
            for i in range(self.config.factorized_k-1):
                idx_i = idx[i]
    ...
    def decode_subtoken(self, h, x, step, input_pos=None):
        ...
        logits = self.head[step](h[:, :, 0, :])

        return logits 


@torch.no_grad()
def sample_WeTok(x, model, steps, temperature=1.0, sample_logits=True, top_k=None, 
    ...
    k = model.config.factorized_k
    bs, _ = x.shape
    device = x.device
    if cfg_scale[0] > 1.0:
        cond_token, uncond_token = torch.split(x, bs // 2, dim=0)
        if k == 2: ## factorization with 2 tokens:
