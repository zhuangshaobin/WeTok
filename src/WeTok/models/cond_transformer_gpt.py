...
from src.WeTok.modules.util import SOSProvider
...
class Net2NetTransformer(L.LightningModule):
    def __init__(self,
        ...
        if permuter_config is None:
            permuter_config = {"target": "src.WeTok.modules.transformer.permuter.Identity"}
    ...
    def forward(self, x, c):
        ...
        if self.token_factorization: ## must be use
            if isinstance(self.first_stage_model.quantize, LFQ):
            ...
            elif isinstance(self.first_stage_model.quantize, GFQ):
                cz_indices = (*a_indices, c_indices) ## AR inside
                target = a_indices
                logits, _ = self.transformer(cz_indices) # [B N M D] where M is the number of factorized tokens
        else:
    ...
    def encode_to_z(self, x):
        ...
        if isinstance(indices, tuple):   # token factorize to 2 tokens
        ...
        elif isinstance(indices, list):# token factorize to more than 2 tokens
            indices = [ind.view(quant_z.shape[0], -1) for ind in indices]
        else:
    ...
    def decode_to_img(self, index, zshape):
        if self.token_factorization: #using token factorization
            if isinstance(self.first_stage_model.quantize, LFQ):
                ...
                quant_z = torch.concat([quant_pre, quant_post], dim=1) # concate in the channel dimension 
            elif isinstance(self.first_stage_model.quantize, GFQ):
                quant_list = []
                for i, factorized_bit in enumerate(self.first_stage_model.quantize.factorized_bits):
                    index_i = index[i]
                    bhwc = (zshape[0], zshape[2], zshape[3], factorized_bit)
                    quant_list.append(self.first_stage_model.quantize.get_codebook_entry(index_i, bhwc, index_order=i))
                quant_z = torch.concat(quant_list, dim=1) # concate in the channel dimension 
    ...
    def shared_step(self, batch, batch_idx):
        ...
        if self.token_factorization:
            ...
            if isinstance(self.first_stage_model.quantize, GFQ):
                loss = 0
                for i, factorized_bits in enumerate(self.first_stage_model.quantize.factorized_bits):
                    logits_i, target_i = logits[i], target[i]
                    loss_i = F.cross_entropy(logits_i.reshape(-1, logits_i.size(-1)), target_i.reshape(-1))
                    loss += loss_i
    ...
    def training_step(self, batch, batch_idx):
        ...
        ## --------------------------------------------
        loss, losses = self.shared_step(batch, batch_idx)
        self.log("train/loss", loss, prog_bar=True, logger=True, on_step=True, on_epoch=True)
        for i, factorized_bits in enumerate(self.first_stage_model.quantize.factorized_bits):
            self.log(f"train/loss_{i}", losses[i], logger=True, on_step=True, on_epoch=True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, factorized_bits in enumerate(self.first_stage_model.quantize.factorized_bits):
            self.log(f"val/loss_{i}", losses[i], logger=True, on_step=True, on_epoch=True)
        return loss
