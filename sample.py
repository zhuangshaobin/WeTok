...
import importlib
from src.WeTok.modules.transformer.gpt import sample_WeTok
import time
try:
    import torch_npu
except:
...
if __name__ == "__main__":
    ...
    if opt.token_factorization:
        logdir = os.path.join(
            logdir, 
            "samples", 
            f"top_k_{opt.top_k[0]}_{opt.top_k[1]}_{opt.top_k[2]}_{opt.top_k[3]}_" +
            f"temp_{opt.temperature[0]:.2f}_{opt.temperature[1]:.2f}_{opt.temperature[2]:.2f}_{opt.temperature[3]:.2f}_" +
            f"top_p_{opt.top_p[0]}_{opt.top_p[1]}_{opt.top_p[2]}_{opt.top_p[3]}_" +
            f"cfg_{opt.cfg_scale[0]}_{opt.cfg_scale[1]}_{opt.cfg_scale[2]}_{opt.cfg_scale[3]}",
            f"{ckpt_name}"
            )
    else:
